...
getstats_triplets=function(phy,gene_trees,clade_name,cl1,cl2)
{
   
    clade1=extract.clade(tt,as.numeric(cl1))$tip.label
    clade2=extract.clade(tt,as.numeric(cl2))$tip.label
    taxa_combn=combn(c(clade1,clade2),3)
    taxa_combn=taxa_combn[,apply(apply(taxa_combn,2,'%in%',clade2),2,sum)==1]
    out_t=c()
...
#Arg 5 node of rooting taxa
getstats_triplets(tt,g_tree,args[3],args[4],args[5])